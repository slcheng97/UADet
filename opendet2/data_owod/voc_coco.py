...
__all__ = ["load_voc_instances_owod", "register_pascal_voc_owod"]
...
def load_voc_instances_owod_v1(dirname: str, split: str, class_names: Union[List[str], Tuple[str, ...]]):
    """
...
def load_voc_instances_owod(dirname: str, split: str, class_names: Union[List[str], Tuple[str, ...]]):
    """
    Load Pascal VOC detection annotations to Detectron2 format.
    ...
    # filter instances
    # if cfg.TEST.MASK == 1:
    #     allowed_class = list(range(0, PREV_INTRODUCED_CLS+CUR_INTRODUCED_CLS))
    # else:
    #     allowed_class = list(range(PREV_INTRODUCED_CLS, PREV_INTRODUCED_CLS+ CUR_INTRODUCED_CLS))
    for id in ids:
...
def register_pascal_voc_owod(name, dirname, super_split, split, year=2007):
    # if "voc_coco" in name:
    #     class_names = VOC_COCO_CLASS_NAMES
    # else:
    #     class_names = tuple(VOC_CLASS_NAMES)
    class_names = VOC_COCO_CLASS_NAMES[super_split]
    DatasetCatalog.register(name, lambda: load_voc_instances_owod(dirname, split, class_names))
