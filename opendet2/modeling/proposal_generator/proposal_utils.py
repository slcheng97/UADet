...
def find_top_rpn_proposals(
    ...
    for n, image_size in enumerate(image_sizes):
        ...
        valid_mask1 = torch.isfinite(boxes.tensor).all(dim=1)
        if not valid_mask1.all():
            if training:
                raise FloatingPointError(
                    "Predicted boxes contain Inf/NaN. Training has diverged."
                )
            boxes = boxes[valid_mask1]
            scores_per_img = scores_per_img[valid_mask1]
            lvl = lvl[valid_mask1]

        valid_mask2 = torch.isfinite(scores_per_img)
        if not valid_mask2.all():
            if training:
                raise FloatingPointError(
                    "Predicted scores contain Inf/NaN. Training has diverged."
                )
            boxes = boxes[valid_mask2]
            scores_per_img = scores_per_img[valid_mask2]
            lvl = lvl[valid_mask2]
